...
from opinion_trading.skills.sentiment_analysis import (
...
class MultiSourceSentimentTests(unittest.TestCase):
    ...
    def test_collect_eastmoney_and_xueqiu_samples(self) -> None:
        mapping = {
            "eastmoney_list": (FIXTURES / "eastmoney_list.html").read_text(
                encoding="utf-8"
            ),
            "eastmoney_article": (FIXTURES / "eastmoney_article.html").read_text(
                encoding="utf-8"
            ),
        ...
        eastmoney_rows = provider.collect_raw_posts(
            "eastmoney", "600519.SH", self.trade_date, max_posts=2
        )
        ...
        xueqiu_rows = provider.collect_raw_posts(
            "xueqiu", "600519.SH", self.trade_date, max_posts=2
        )
